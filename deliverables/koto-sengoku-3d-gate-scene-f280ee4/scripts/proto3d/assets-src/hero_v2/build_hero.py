...
第 2 版（3D 比較版の肩越しカメラ向けの改良）。元は第 1 版（proto3d/assets-src/hero_v1/source/build_hero.py、利用者が用意）。
骨組み・動き（Idle / Walk / Run）は第 1 版と同じ。変えたのは、背中から近くで見える所：
- 袴：色を少し明るく暖かい灰茶に、細い縦縞を入れる（黒い塊に見えないように）
- 小袖：首の後ろを回る襟と、その上に少しのぞく白い半襟を足す（背中の首元が開いて見えないように）
- 首から肩の付け根の肌を、後ろ側だけ小袖の内側に収める（背中に肌が突き出して見えていた）
- 袴の腰板（背中の腰の台形の板）と縁を足す
- 袖のふくらみを少し小さく（風船のように見えないように）
出力はこのフォルダの assets/ に書く。ゲームでは assets/sengoku_hero.glb を proto3d/public/models/hero_v2.glb として使う。
"""
...
def cloth_texture(name, base, strength=.08, direction=0, stripes=0.0):
    n=512
    ...
    if stripes:
        # 細い縦縞（袴の縞）：u の向き（周り）に 512px で 56 本ほど。糸の太さのむらも少し
        stripe=np.clip((np.cos(xx*2*PI/9.14)-.72)/.28,0,1)*(1+.3*np.sin(yy*.05+xx*.01))
        val=val*(1-stripes*.35)+stripe*stripes
...
cloth_texture('hakama',(84,77,66),.6,stripes=.16)
...
def neck_surface(u,v):
    ...
    # 第 2 版：首から肩の付け根の肌は前（襟の開き）だけ見えればよい。後ろは小袖より内側に収める（背中に肌が突き出ないように）
    back=1.0 if math.cos(a)>=0 else 1-.22*float(np.clip((1.47-y)/.1,0,1))
    return (float(neck_x(y))*math.sin(a),y,float(neck_z(y))*math.cos(a)*back-.002)
...
for s,side in [(1,'Left'),(-1,'Right')]:
    ...
    radius=PchipInterpolator([0,.18,.44,.66,.82,1],[.054,.069,.075,.071,.061,.043])
    def sleeve(u,v):
...
# 第 2 版：首の後ろを回る襟（小袖と同じ布）と、その上にのぞく白い半襟。前の襟の端につながる角度から、背中を回る
def back_collar(r0,r1,y0,y1,lean):
    def fn(u,v):
        a=.9+(2*PI-1.8)*u
        y=y0+(y1-y0)*v
        k=1-lean*v
        return (r0*k*math.sin(a),y,r1*k*math.cos(a)-.004)
    return fn
surface('back collar',back_collar(.066,.061,1.428,1.482,.12),40,6,'Indigo woven kosode','Chest',uscale=1.2,vscale=.3)
surface('back half collar',back_collar(.059,.054,1.45,1.495,.1),40,5,'Undercollar and tabi','Chest',uscale=1.2,vscale=.3)
# 第 2 版：袴の腰板（背中の腰の、下が広い台形の板。体に沿って少し曲げる）と縁
def koshiita(u,v):
    y=.968+.118*v
    half=.118-.022*v
    x=(u-.5)*2*half
    return (x,y,-(.126-3.2*x*x*.3)-.004*v)
surface('koshi-ita',koshiita,16,6,'Pleated charcoal hakama','Hips',uscale=.5,vscale=.3)
rim=[koshiita(u,0) for u in np.linspace(0,1,9)]+[koshiita(1,v) for v in np.linspace(0,1,5)][1:]+[koshiita(u,1) for u in np.linspace(1,0,9)][1:]+[koshiita(0,v) for v in np.linspace(1,0,5)][1:]
tube('koshi-ita rim',rim,.0028,'Seam grey','Hips',6)
