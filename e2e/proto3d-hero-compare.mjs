...
async function open(query, opts = {}) {
  ...
  await page.goto(`${BASE}/?q=low&${query}`);
...
// 切り替えボタン：本物のクリック（PC）とタップ（スマホ）。位置・向き・歩く／走るはそのまま
{
  ...
  await page.click('#hero-btn');
  await page.waitForFunction(() => window.__p3.heroModel === 'v1', null, { timeout: 60000 });
  check('PC：右上のボタンをクリックすると新／旧が替わり、位置・向き・「走る」の選択はそのまま', a.m === 'v1' && b.m === 'old' && b.on && a.x === b.x && a.h === b.h && a.run && b.run);
  await ctx.close();
}
{
...
<div style="display:grid;grid-template-columns:640px 640px;gap:6px 16px;margin-bottom:8px"><div>新：自作の主人公モデル 第 1 版</div><div>旧：これまでの主人公</div></div>
${labels.map((l) => `<div style="margin:12px 0 4px">${l}</div><div style="display:grid;grid-template-columns:640px 640px;gap:16px">${['v1', 'old'].map((k) => `<img src="${img(rows.find((r) => r[0] === l && r[1] === k)[2])}" style="width:640px;border-radius:4px">`).join('')}</div>`).join('')}
</div></body>`);
